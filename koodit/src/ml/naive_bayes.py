...
class NaiveBayes:
    ...
    def preprocess_word(word:str):
        ...
        # this goes through each character in the word, checks if it is alphanumeric, and if it is, it adds it to the new word
        return word
    ...
    def predict(self, evidence:str, verbose=True):
        ...
        predictions: list[float] = [
            # IMPLEMENT the formula here for class 0
            self.priors[0] * prod(self.word_likelihoods[word][0] for word in evidence.split() if word in self.word_likelihoods),
            # IMPLEMENT the formula here for class 1
